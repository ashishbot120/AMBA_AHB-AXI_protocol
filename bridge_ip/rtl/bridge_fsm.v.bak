...
localparam S_RD_LATCH = 3'd5;   // extra cycle to latch read data

reg [2:0]  state;
reg [31:0] addr_reg, wdata_reg, rdata_reg;
reg        write_reg;
reg [2:0]  ahb_size_reg;
reg [3:0]  wstrb_reg;
reg [1:0]  resp_reg;
    ...
    if (!HRESETn) begin
        ...
        case (state)
            ...
            S_WAIT: begin
                if (axi_done) begin
                    if (write_reg) begin
                        // Write — resp valid same cycle as done
                        rdata_reg <= 32'b0;
                        resp_reg  <= axi_resp;
                        state     <= S_COMPLETE;
                    end
                    else begin
                        // Read — axi_rdata registered same clock as axi_done
                        // needs one extra cycle to settle
                        state <= S_RD_LATCH;
                    end
                end
            end
            ...
            S_RD_LATCH: begin
                // axi_rdata and axi_resp now fully valid
                rdata_reg <= axi_rdata;
                resp_reg  <= axi_resp;
                state     <= S_COMPLETE;
            end
